...
Particularly, one can take many discrete models (e.g. sticky tetris) and associate to them a 'height' function over the real line and let it evolve in time. One can then consider rescaled height functions <br> 
(with scaling \\(\varepsilon h(\varepsilon^{-3} t,\varepsilon^{-2} x)\\) where \\(h(t,x)\\) is the original height function, i.e. zooming out as 
\\(\varepsilon\to 0\\)) which are conjectured to converge a universal object, namely the KPZ fixed point. This convergence has been proved for a limited number of models so far, (sticky tetris not being one of them!) and mathematicians are actively working on this.
